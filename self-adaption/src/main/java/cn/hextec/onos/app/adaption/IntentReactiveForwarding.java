...
/**
 * 意图活性转发 WORK-IN-PROGRESS: Sample reactive forwarding application using intent framework.
 *
 * @author yeon
 * @date 2020-09-08
 */
...
public class IntentReactiveForwarding {
  ...
  private static final EnumSet<IntentState> WITHDRAWN_STATES =
      EnumSet.of(IntentState.WITHDRAWN, IntentState.WITHDRAWING, IntentState.WITHDRAW_REQ);

  /** 激活 */
  @Activate
  public void activate() {
    appId = coreService.registerApplication("cn.hextec.onos.app.adaption");
  ...
  private void flood(PacketContext context) {
    if (topologyService.isBroadcastPoint(
        topologyService.currentTopology(), context.inPacket().receivedFrom())) {
  ...
  private void forwardPacketToDst(PacketContext context, Host dst) {
    ...
    OutboundPacket packet =
        new DefaultOutboundPacket(
            dst.location().deviceId(), treatment, context.inPacket().unparsed());
